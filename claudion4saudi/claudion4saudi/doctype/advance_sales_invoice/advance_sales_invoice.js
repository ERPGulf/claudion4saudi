...
// erpnext.accounts.taxes.setup_tax_validations("Advance Sales Invoice");
...
frappe.ui.form.on("Advance Sales Invoice", {
  ...
  paid_amount: function (frm) {
    ...
    // Calculate base_paid_amount based on source_exchange_rate
    frm.set_value(
      "base_paid_amount",
      flt(frm.doc.paid_amount) * flt(frm.doc.source_exchange_rate)
    );
    ...
    // Calculate received_amount and base_received_amount
    if (
      frm.doc.paid_from_account_currency === frm.doc.paid_to_account_currency
    ) {
      frm.set_value("received_amount", frm.doc.paid_amount);
