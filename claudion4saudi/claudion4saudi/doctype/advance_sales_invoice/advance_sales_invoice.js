...
frappe.ui.form.on("Advance Sales Invoice", {
  ...
  setup: function (frm) {
    frm.set_query("paid_from", function () {
      ...
      var account_types = ["Pay", "Internal Transfer"].includes(
    ...
    frm.set_query("payment_term", "references", function (frm, cdt, cdn) {
      ...
      if (
        ["Purchase Invoice", "Sales Invoice"].includes(
    ...
    frm.set_query("payment_request", "references", function (doc, cdt, cdn) {
      ...
      return {
        query:
          "erpnext.accounts.doctype.payment_request.payment_request.get_open_payment_requests_query",
        filters: {
    ...
    erpnext.accounts.pos.get_payment_mode_account(
      ...
      function (account) {
        let payment_account_field =
          frm.doc.payment_type == "Receive" ? "paid_to" : "paid_from";
  ...
  party: function (frm) {
    ...
    if (
      ...
      if (!frm.doc.posting_date) {
        frappe.msgprint(
          __("Please select Posting Date before selecting Party")
        );
        frm.set_value("party", "");
        return;
      }
  ...
  set_current_exchange_rate: function (
    frm,
    exchange_rate_field,
    from_currency,
    to_currency
    ...
    frappe.call({
      ...
      callback: function (r, rt) {
        const ex_rate = flt(
          r.message,
          frm.get_field(exchange_rate_field).get_precision()
        );
          ...
          $.each(r.message, function (i, d) {
            ...
            if (!in_list(frm.events.get_order_doctypes(frm), d.voucher_type)) {
              if (flt(d.outstanding_amount) > 0)
                ...
                total_negative_outstanding += Math.abs(
            ...
            if (
              in_list(frm.events.get_invoice_doctypes(frm), d.reference_doctype)
            ) {
              c.due_date = d.due_date;
            }
          });
  ...
  check_mandatory_to_fetch: function (frm) {
    $.each(
      ["Company", "Party Type", "Party", "payment_type"],
      function (i, field) {
  ...
  validate_reference_document: function (frm, row) {
    var _validate = function (i, row) {
      ...
      if (
        frm.doc.party_type == "Customer" &&
        !["Sales Order", "Sales Invoice", "Journal Entry", "Dunning"].includes(
  ...
  validate_taxes_and_charges: function (d) {
    ...
    if (!d.charge_type && (d.row_id || d.rate || d.tax_amount)) {
      ...
      (d.charge_type == "On Previous Row Amount" ||
      ...
      if (d.idx == 1) {
        ...
        msg = __("Please specify a valid Row ID for row {0} in table {1}", [
          d.idx,
          __(d.doctype),
        ]);
        d.row_id = "";
  ...
  determine_exclusive_rate: function (frm) {
    ...
    $.each(frm.doc["taxes"] || [], function (i, tax) {
      tax.tax_fraction_for_current_item = frm.events.get_current_tax_fraction(
        frm,
        tax
      );
      ...
      frm.doc.paid_amount_after_tax = flt(
        frm.doc.base_paid_amount / (1 + cumulated_tax_fraction)
      );
    });
  },
  ...
  get_current_tax_fraction: function (frm, tax) {
    ...
    if (cint(tax.included_in_paid_amount)) {
      ...
      if (tax.charge_type == "On Paid Amount") {
        ...
        current_tax_fraction =
          (tax_rate / 100.0) *
          frm.doc["taxes"][cint(tax.row_id) - 1]
            .grand_total_fraction_for_current_item;
      }
    }
  ...
  calculate_taxes: function (frm) {
    ...
    $.each(frm.doc["taxes"] || [], function (i, tax) {
      ...
      if (i == 0) {
        tax.total = flt(
          frm.doc.paid_amount_after_tax + current_tax_amount,
          precision("total", tax)
        );
      } else {
        tax.total = flt(
      ...
      if (frm.doc.payment_type == "Pay") {
        ...
        if (tax.currency != frm.doc.paid_to_account_currency) {
          ...
          frm.doc.total_taxes_and_charges += flt(
            current_tax_amount / frm.doc.target_exchange_rate
          );
        } else {
        ...
        if (tax.currency != frm.doc.paid_from_account_currency) {
          ...
          frm.doc.total_taxes_and_charges += flt(
            current_tax_amount / frm.doc.source_exchange_rate
          );
        } else {
          frm.doc.total_taxes_and_charges += current_tax_amount;
        }
  ...
  get_current_tax_amount: function (frm, tax) {
    ...
    if (
      ["On Previous Row Amount", "On Previous Row Total"].includes(
        tax.charge_type
      )
    ) {
      if (tax.idx === 1) {
    ...
    if (tax.charge_type == "Actual") {
      ...
      current_tax_amount = flt(
        (tax_rate / 100.0) * frm.doc.paid_amount_after_tax
      );
    } else if (tax.charge_type == "On Previous Row Amount") {
      current_tax_amount = flt(
        (tax_rate / 100.0) * frm.doc["taxes"][cint(tax.row_id) - 1].tax_amount
      );
    } else if (tax.charge_type == "On Previous Row Total") {
      current_tax_amount = flt(
        (tax_rate / 100.0) * frm.doc["taxes"][cint(tax.row_id) - 1].total
      );
    }
